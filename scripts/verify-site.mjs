...
async function verifyHeaders(url) {
  ...
  for (const check of checks) {
    const value =
      headers.get(check.key) || headers.get(check.key + "-report-only");
    if (value) {
      if (check.expected && value !== check.expected) {
        console.log(
          `[FAIL] ${check.name}: Expected "${check.expected}", got "${value}"`,
        );
        allPassed = false;
      } else {
...
async function verifySEO(url) {
  ...
  if (ogTitle) console.log(`[PASS] og:title: ${ogTitle}`);
    ...
    console.log(`[FAIL] og:title: Missing`);
    allPassed = false;
  }

  if (ogDescription) console.log(`[PASS] og:description: ${ogDescription}`);
  else {
    console.log(`[FAIL] og:description: Missing`);
    allPassed = false;
  }
...
async function verifySitemap() {
  ...
  if (res.status === 200) {
    const text = await res.text();
    const urls = ["/projects/cpu-grid-traffic", "/projects/portfolio-next"];
    let allFound = true;
